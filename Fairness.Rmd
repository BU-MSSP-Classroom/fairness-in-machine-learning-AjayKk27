...
~~~
The favorable label is getting a recidivism risk of 'low' or violent recidivism risk of 'low'. A favorable label might grand the user a better chance at parole. Non punitive but also assertive. 
...
~~~
False positive rate parity: We want to look at some sort of predictive parity metric because we are comparing predicted output to actual results. The big issue here is whether the output predicts that someone will recidivate when they won't disporportionately for different races: So we look at false positives.
...
```{r}
#
compas_recid<- compas_df |> mutate(pred_recid=ifelse(score_text=="Low",0,1))
res_fpr <- fpr_parity(data         = compas_recid, 
